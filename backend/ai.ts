...
function createClient() {
  ...
  try {
    return new GoogleGenAI({
      ...
      httpOptions: {
        headers: {
          'User-Agent': 'family-budget-tracker',
        },
      },
    });
  } catch (error) {
...
export async function generateBudgetAnalysis(expenses: ExpenseLike[], allowance = 15000, currentMonth = 'this month') {
  ...
  const categoriesText = Object.entries(categoryMap)
    .map(([category, amount]) => `- ${category}: ${amount} Birr`)
    .join('\n');

  const expensesText = expenses
    .map((entry) => `- ${entry.date}: ${entry.amount} Birr for "${entry.description}" (${entry.category})`)
    .join('\n');
